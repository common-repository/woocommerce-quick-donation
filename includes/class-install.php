    ...
    public static function check_template_files(){
        ...
        if(is_array($template_files)){
            ...
            if ( $outdated ) {
                $theme = wp_get_theme(); 
                $message = sprintf(
                    __( '<p> <strong>Your theme (%s) contains outdated copies of some Quick Donation For WooCommerce template files.</strong> These files may need updating to ensure they are compatible with the current version of Quick Donation For WooCommerce. You can see which files are affected from the %ssystem status page%s. If in doubt, check with the author of the theme. </p> <p class="submit">%sLear More About Templates%s %s</p>',
                      WC_QD_TXT), 
                    esc_html( $theme['Name'] ), 
